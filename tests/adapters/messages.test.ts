...
import { generateTestId, assertSuccessfulSendResult } from '../utils';
...
describe('Message Classes', () => {
  ...
  describe('Email Message', () => {
    ...
    test('should handle CC and BCC recipients', () => {
      ...
      expect(email.getCC()).toEqual([{ name: 'CC User', email: 'cc@example.com' }]);
      expect(email.getBCC()).toEqual([{ email: 'bcc@example.com' }]);
    });

    test('should handle attachments', () => {
      const attachment = new Attachment('test.txt', Buffer.from('test'), 'text/plain');
      const email = new EmailMessage({
        to: ['test@example.com'],
        subject: 'Test Subject',
        content: 'Test content',
  ...
  describe('SMS Message', () => {
    ...
    test('should create SMS with array constructor', () => {
      const sms = new SMSMessage(['+1234567890'], 'Test SMS content', '+0987654321');
...
describe('Base Adapter Functionality', () => {
  ...
  class TestAdapter extends Adapter {
    constructor(private testName: string = 'TestAdapter') {
      super();
    }
    ...
    public async testRequest(method: string, url: string, headers?: Record<string, string>, body?: any) {
  ...
  test('should handle invalid phone numbers', () => {
    ...
    expect(() => adapter.testGetCountryCode('')).toThrow('$phone cannot be empty.');
