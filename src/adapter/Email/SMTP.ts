...
import type { Email as EmailMessage } from "../../messages/Email";
...
export class SMTP extends EmailAdapter {
		...
		this.transporter = nodemailer.createTransport({
			...
			auth:
				this.username && this.password
					...
					: undefined,
			tls:
				this.smtpSecure === "tls" ? { rejectUnauthorized: false } : undefined,
	...
	protected async process(message: EmailMessage) {
		...
		if (message.getTo().length === 0) {
			if (
				message.getBCC() &&
				message.getBCC()!.length === 0 &&
				!message.getDefaultRecipient()
			) {
				throw new Error(
					'Email requires either "to" recipients or both BCC and a default recipient configurations',
				);
			}
		...
		if (message.getCC()) {
			mailOptions.cc = message
				.getCC()
				?.map((cc) => `${cc.name || ""} <${cc.email}>`);
		}

		if (message.getBCC()) {
			mailOptions.bcc = message
				.getBCC()
		...
		try {
			...
			const totalDelivered =
				...
				message.getCC()!.length +
				message.getBCC()!.length;
			...
			message
				.getCC()!
				.forEach((cc) => response.addResult(cc.email, errorMessage));
			message
				.getBCC()!
				.forEach((bcc) => response.addResult(bcc.email, errorMessage));
