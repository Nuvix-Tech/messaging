...
export class FCM extends PushAdapter {
    ...
    protected static readonly GOOGLE_TOKEN_URL = 'https://www.googleapis.com/oauth2/v4/token';
    ...
    protected async process(message: PushMessage) {
        ...
        if (message.getAction() !== null) {
            ...
            shared.message.android.notification = shared.message.android.notification || {};
        ...
        if (message.getCritical() !== null) {
            ...
            shared.message.apns.payload = shared.message.apns.payload || {};
            shared.message.apns.payload.aps = shared.message.apns.payload.aps || {};
            shared.message.apns.payload.aps.sound = shared.message.apns.payload.aps.sound || {};
        ...
        if (message.getContentAvailable() !== null) {
            ...
            shared.message.apns.payload = shared.message.apns.payload || {};
            shared.message.apns.payload.aps = shared.message.apns.payload.aps || {};
            shared.message.apns.payload.aps['content-available'] = Number(message.getContentAvailable());
        }
        if (message.getPriority() !== null) {
            shared.message = shared.message || {};
            shared.message.android = shared.message.android || {};
            shared.message.android.priority = message.getPriority() === Priority.HIGH ? 'high' : 'normal';

            shared.message.apns = shared.message.apns || {};
            shared.message.apns.headers = shared.message.apns.headers || {};
            shared.message.apns.headers['apns-priority'] = message.getPriority() === Priority.HIGH ? '10' : '5';
        }
